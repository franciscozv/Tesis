...
export function useVincularMiembro() {
  ...
  return useMutation({
    mutationFn: (input: VincularMiembroInput) => integranteCuerpoApi.vincular(input),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: [INTEGRANTE_CUERPO_QUERY_KEY, 'grupo', variables.grupo_id],
      });
      queryClient.invalidateQueries({
        queryKey: [INTEGRANTE_CUERPO_QUERY_KEY, 'miembro', variables.miembro_id],
      });
      queryClient.invalidateQueries({
