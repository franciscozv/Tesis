              ...
              const filas = [...selecciones.entries()].map(([cargoId, miembroId]) => {
                ...
                if (otrosCargosEnBatch.length > 0) {
                  const nombresOtros = otrosCargosEnBatch
                    .map(
                      (id) => rolesDirectiva.find((r) => r.id_rol_grupo === id)?.nombre ?? `#${id}`,
                    )
                    .join(', ');
                  advertencias.push({
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex gap-1">
                                    <Badge
                                      variant="outline"
                                      className="h-4 px-1 text-[9px] font-normal gap-0.5 border-muted-foreground/30 bg-muted/50 text-muted-foreground"
                                    >
                                    ...
                                    <Badge
                                      variant="outline"
                                      className="h-4 px-1 text-[9px] font-normal gap-0.5 border-success-foreground/40 bg-success text-success-foreground"
                                    >
                                ...
                                {c.indicadores.historial_experiencia &&
                                  c.indicadores.historial_experiencia.length > 0 && (
                                      ...
                                      {c.indicadores.historial_experiencia
                                        .map((h) => {
                                          ...
                                          const fin = h.fecha_fin
                                            ? new Date(h.fecha_fin).getFullYear()
                                            : 'Hoy';
                                ...
                                {c.indicadores.historial_otros_cargos &&
                                  c.indicadores.historial_otros_cargos.length > 0 && (
                                    <p className="text-[10px] text-muted-foreground leading-tight text-left">
                                      Otros cargos:{' '}
                                      {c.indicadores.historial_otros_cargos
                                        .map((h) => {
                                ...
                                {c.indicadores.grupos_activos_detalle &&
                                  c.indicadores.grupos_activos_detalle.length > 0 && (
                                    <p className="text-[10px] text-muted-foreground leading-tight text-left">
                                      Participa en:{' '}
                                      {c.indicadores.grupos_activos_detalle
                                        .map((g) => `${g.grupo} (${g.rol})`)
                                ...
                                <p className="mt-1 text-left text-[10px] leading-tight text-muted-foreground">
                                  Fidelidad: {pct}% asistencia ({c.indicadores.asistencias_count} de{' '}
                                  {c.indicadores.confirmadas_count})
                                </p>
                                <p className="text-[10px] text-muted-foreground leading-tight text-left">
                                  Antigüedad: {c.indicadores.antiguedad_anios} años (Desde{' '}
