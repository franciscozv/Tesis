...
import { type Responsibility, getColumns } from "./columns";
...
const Page = () => {
	...
	const handleSave = async (responsibilityId: number) => {
		const responsibilityToUpdate = responsibilities.find(
			(r) => r.id === responsibilityId,
		);
		if (responsibilityToUpdate) {
			const validationResult = responsibilitySchema.safeParse(
