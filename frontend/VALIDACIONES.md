...
```typescript
// Backend Zod
email: z.string().email().max(150)    // miembros / cuenta
```
...
| Max caracteres | 150 |
| Requerido | Si (al crear miembro) |
| Unicidad | Unico en BD (tabla `miembro`) |
...
```typescript
// Backend Zod
password: z.string().min(8).max(100)       // crear cuenta / reset
...
| `cantidad_comprometida` | number | >0 | — | Colaboradores |
...
cantidad_comprometida <= cantidad_faltante             (al crear colaboración)
cantidad_faltante = cantidad_requerida - cantidad_cubierta
```

- Al **aceptar** una colaboración, se suma `cantidad_comprometida` a `cantidad_cubierta` automáticamente.
...
| Campo | Descripcion |
|-------|-------------|
| `id` | ID del miembro (cuenta unificada) |
| `email` | Email del miembro |
| `rol` | Rol del miembro |
| Expiracion | Configurada por variable de entorno |
...
const CreateColaboradorSchema = z.object({
  ...
  miembro_id: z.number().int().positive(),
  cantidad_comprometida: z.number().positive(),
