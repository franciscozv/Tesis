...
export class ResponsibilityService {
  ...
  constructor(
    repository: ResponsibilityRepository = new ResponsibilityRepository()
  ) {
  ...
  async findAll(): Promise<ServiceResponse<Responsibility[] | null>> {
    try {
      const responsibilities =
        await this.responsibilityRepository.findAllAsync();
      ...
      return ServiceResponse.success<Responsibility[]>(
        "Responsibilities found",
        responsibilities
      );
    } catch (ex) {
      const errorMessage = `Error finding all responsibilities: ${
        (ex as Error).message
      }`;
  ...
  async create(
    data: CreateResponsibilityInput
  ): Promise<ServiceResponse<Responsibility | null>> {
    try {
      const responsibility = await this.responsibilityRepository.createAsync(
        data
      );
      if (!responsibility) {
        return ServiceResponse.failure(
      ...
      const errorMessage = `Error creating responsibility: ${
        (ex as Error).message
      }`;
      logger.error(errorMessage);
      return ServiceResponse.failure(
  ...
  async deleteById(id: number): Promise<ServiceResponse<null>> {
    try {
      const deletedResponsibility =
        await this.responsibilityRepository.deleteByIdAsync(id);
      ...
      const errorMessage = `Error deleting responsibility with id ${id}: ${
        (ex as Error).message
      }`;
      logger.error(errorMessage);
      return ServiceResponse.failure(
  ...
  async updateById(
    ...
    try {
      const updatedResponsibility =
        await this.responsibilityRepository.updateByIdAsync(id, data);
      ...
      const errorMessage = `Error updating responsibility with id ${id}: ${
        (ex as Error).message
      }`;
      logger.error(errorMessage);
      return ServiceResponse.failure(
